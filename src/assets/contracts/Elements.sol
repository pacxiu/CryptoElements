...
contract Elements is ERC721 {
	...
	uint256 private secondStepLimit = 1 ether;
	...
	/// @dev A mapping from ElementIDs to the previpus price of the token. Used
	/// to calculate price delta for payouts
	mapping (uint256 => uint256) private elementIndexToPreviousPrice;

	// @dev A mapping from elementId to the 7 last owners.
	mapping (uint256 => address[5]) private elementIndexToPreviousOwners;
	...
	/// @dev Creates a new Element with the given name.
	function createContractElement(string _name) public {
	_createElement(_name, address(this), elementStartingPrice);
	}
	...
	previousPrice = elementIndexToPreviousPrice[_tokenId];
	previousOwners = elementIndexToPreviousOwners[_tokenId];
	}
	...
	address[5] storage previousOwners = elementIndexToPreviousOwners[_tokenId];

	uint256 sellingPrice = elementIndexToPrice[_tokenId];
	uint256 previousPrice = elementIndexToPreviousPrice[_tokenId];
	...
	uint256 ownerPayout = SafeMath.add(previousPrice, SafeMath.mul(SafeMath.div(priceDelta, 100), 80));
	...
	if (sellingPrice < firstStepLimit) {
	  ...
	  // third stage
	  elementIndexToPrice[_tokenId] = SafeMath.div(SafeMath.mul(sellingPrice, 135), 100);
	} else {
	  // fourth stage
	  elementIndexToPrice[_tokenId] = SafeMath.div(SafeMath.mul(sellingPrice, 120), 100);
	}
	elementIndexToPreviousPrice[_tokenId] = sellingPrice;
	...
	if (oldOwner != address(this)) {
	  ...
	  fee_for_dev = SafeMath.mul(SafeMath.div(priceDelta, 100), 5);
	} else {
	  fee_for_dev = SafeMath.add(ownerPayout, SafeMath.mul(SafeMath.div(priceDelta, 100), 5));
	}

	// Next distribute payout Total among previous Owners
	for (uint i = 0; i <= 4; i++) {
	    if (previousOwners[i] != address(this)) {
	        previousOwners[i].transfer(uint256(SafeMath.div(SafeMath.mul(priceDelta, 3), 100)));
	    } else {
	        fee_for_dev = SafeMath.add(fee_for_dev, uint256(SafeMath.div(SafeMath.mul(priceDelta, 3), 100)));
	    }
	}
	...
	if (tokenCount == 0) {
	  ...
	  for (elementId = 0; elementId <= totalElements; elementId++) {
	    if (elementIndexToOwner[elementId] == _owner) {
	...
	function _createElement(string _name, address _owner, uint256 _price) private returns (string) {
	...
	elementIndexToPreviousPrice[newElementId] = 0;
	elementIndexToPreviousOwners[newElementId] =
	...
	// Update the elementIndexToPreviousOwners
	elementIndexToPreviousOwners[_tokenId][4] = elementIndexToPreviousOwners[_tokenId][3];
	elementIndexToPreviousOwners[_tokenId][3] = elementIndexToPreviousOwners[_tokenId][2];
	elementIndexToPreviousOwners[_tokenId][2] = elementIndexToPreviousOwners[_tokenId][1];
	elementIndexToPreviousOwners[_tokenId][1] = elementIndexToPreviousOwners[_tokenId][0];
	// the _from address for creation is 0, so instead set it to the contract address
	if (_from != address(0)) {
	    elementIndexToPreviousOwners[_tokenId][0 ] =_from;
	} else {
	    elementIndexToPreviousOwners[_tokenId][0] = address(this);
	}
