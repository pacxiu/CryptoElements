...
contract Elements is ERC721 {
  ...
  uint256 private thirdStepLimit = 1.75 ether;
  ...
  function createContractElement(string _name) public {
    _createElement(_name, address(this), startingPrice);
  }
  ...
  function purchase(uint256 _tokenId) public payable {
    ...
    uint256 ownerPayout = SafeMath.add(previousPrice, SafeMath.mul(SafeMath.div(priceDelta, 100), 72));
    ...
    if (oldOwner != address(this)) {
      ...
      fee_for_dev = SafeMath.mul(SafeMath.div(priceDelta, 100), 3);
    } else {
      fee_for_dev = SafeMath.add(ownerPayout, SafeMath.mul(SafeMath.div(priceDelta, 100), 3));
    }

    // Next distribute payout Total among previous Owners
    for (uint i = 0; i <= 4; i++) {
        if (previousOwners[i] != address(this)) {
            previousOwners[i].transfer(uint256(SafeMath.div(SafeMath.mul(priceDelta, 5), 100)));
        } else {
            fee_for_dev = SafeMath.add(fee_for_dev, uint256(SafeMath.div(SafeMath.mul(priceDelta, 5), 100)));
        }
    }
